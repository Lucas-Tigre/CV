...
function updatePhysics(deltaTime) {
    ...
    if (state.enemies.length > 0) {
        ...
        // A função updateEnemies agora retorna um objeto estruturado.
        // É crucial verificar se o retorno é válido antes de desestruturar.
        if (enemyUpdate) {
            state.setEnemies(enemyUpdate.newEnemies);

            if (enemyUpdate.newlyCreatedParticles && enemyUpdate.newlyCreatedParticles.length > 0) {
            ...
            if (enemyUpdate.xpFromDefeatedEnemies > 0) {
                ...
                config.xp += finalXp;
                updateQuest('defeat20', 1); // Assumindo que a missão é derrotar inimigos 1 a 1
                checkLevelUp();
            }
        }
    }
